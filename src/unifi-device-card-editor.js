  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="wrap">
        ...
        <div class="field">
          <label for="device">UniFi Gerät</label>
          ${this._loading
        ...
        <div class="field">
          ...
          <input id="name" type="text" value="${selName}"
            placeholder="Optional — wird sonst vom Gerät übernommen" />
        </div>

        ${this._error ? `<div class="error">${this._error}</div>` : ""}

        ${!this._loading && !this._devices.length && !this._error
          ? `<div class="hint">Keine UniFi Switches oder Gateways in Home Assistant gefunden.</div>`
          : !this._loading
            ? `<div class="hint">Nur Geräte aus der UniFi Network Integration werden angezeigt.</div>`
            : ""}
