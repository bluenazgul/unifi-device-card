/* UniFi Device Card 0.0.0-dev.db124cd */
...
function classifyDevice(device, entities) {
  ...
  if (hasUbiquitiManufacturer(device)) {
    ...
    if (model.includes("udm") || model.includes("ucg") || model.includes("uxg") || model.includes("ugw") || name.includes("gateway")) return "gateway";
...
function applyWanPortOverride(wanPort, specials, numbered, layout) {
  ...
  if (targetIdx !== -1) {
    ...
    const oldWan2 = oldWanIdx2 !== -1 ? { ...newSpecials[oldWanIdx2] } : null;
...
var init_helpers = __esm({
  "src/helpers.js"() {
    init_model_registry();
    SWITCH_MODEL_PREFIXES = ["USW", "USL", "USF", "US8", "US16", "US24", "US48", "USMINI", "FLEXMINI"];
    GATEWAY_MODEL_PREFIXES = ["UDM", "UCG", "UXG", "UGW", "UDRULT", "UDMPRO", "UDMPROSE"];
