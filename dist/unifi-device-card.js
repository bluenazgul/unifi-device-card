/* UniFi Device Card 0.0.0-dev.1df239b */
...
var UnifiDeviceCard = class extends HTMLElement {
  ...
  _isOddEvenExcludedModel(ctx) {
    const model = String(ctx?.model || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
    return model.includes("USPM16") || model.includes("PROMAX16");
  }
  _shouldUseOddEvenRows(ctx, numbered) {
    if (!ctx || ctx.type !== "switch" && ctx.type !== "gateway") return false;
    if (this._isOddEvenExcludedModel(ctx)) return false;
